  ...
  delivery: {
    ...
    mockPoints: [
      ...
      },
      {
        ...
        type: 'relay',
        hours: '09:00–18:30',
        label: 'Relais La Poste Centre — 18 Rue de la République, 34000 Montpellier',
      },
      {
        id: 'MR-003',
        name: 'Relais Épicerie du Port',
        ...
        type: 'relay',
        hours: '08:00–20:00',
        label: 'Relais Épicerie du Port — 42 Avenue du Mondial, 34000 Montpellier',
      },
    ],
  },
};
...
const state = {
  ...
  // Delivery — always Mondial Relay / point relais
...
function initEnhancedDeliveryModule() {
  // Delivery is always Mondial Relay / point relais — no mode or carrier selection needed.
  // Pickup panel auto-loads when address fields complete (wired in initInlineValidation).

  const refreshBtn = document.getElementById('pickupRefreshBtn');
  refreshBtn?.addEventListener('click', () => loadPickupPoints());
}
...
async function loadPickupPoints() {
  ...
  try {
    ...
    if (!points.length) {
      setPickupEmpty(true, 'Aucun point relais trouvé pour cette adresse.');
      renderPickupList([]);
      destroyPickupMarkers();
...
async function fetchPickupPointsMockAware() {
  // Hook to replace with real API / backend proxy:
  // return await fetch('/api/pickup-points?zip=...&city=...').then(r => r.json());

  const seeded = CONFIG.delivery.mockPoints.map((point, idx) => ({
    ...point,
...
function renderPickupSummary() {
  ...
  if (meta) meta.textContent = `${state.pickupPoint.address}, ${state.pickupPoint.zip} ${state.pickupPoint.city} · ${fmt.km(state.pickupPoint.distanceKm)}`;

  const titleEl = document.getElementById('pickupSummaryTitle');
  const addrEl  = document.getElementById('pickupSummaryAddress');
  const distEl  = document.getElementById('pickupSummaryDistance');
  if (titleEl) titleEl.textContent = state.pickupPoint.name;
  if (addrEl)  addrEl.textContent  = `${state.pickupPoint.address}, ${state.pickupPoint.zip} ${state.pickupPoint.city}`;
  if (distEl)  distEl.textContent  = fmt.km(state.pickupPoint.distanceKm);
}
...
function syncDeliveryHiddenFields() {
  ...
  // Always forced — Mondial Relay / point relais only
  if (carrierCodeEl)  carrierCodeEl.value  = 'mondial_relay';
  if (carrierNameEl)  carrierNameEl.value  = 'Mondial Relay';
  if (deliveryTypeEl) deliveryTypeEl.value = 'relay';
