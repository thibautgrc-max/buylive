...
const state = {
  ...
  pickupName:    'Tabac Presse du Centre',
  pickupAddress: '12 Rue de la République, 75001 Paris',
...
function render() {
  ...
  const pairs = {
    ...
    hiddenPickupPoint:      state.deliveryMode === 'mondial-relay'
...
function initDeliveryChoice() {
  ...
  const pickupShell = $id('pickupShell');
  ...
  function selectMode(mode) {
    ...
    /* Pickup shell : visible uniquement si Mondial Relay */
    if (pickupShell) {
      pickupShell.hidden = mode !== 'mondial-relay';
    }
  ...
  /* ── Points relais — sélection interactive ── */
  const pickupCards = document.querySelectorAll('.pickup-card[data-pickup-id]');

  function selectPickup(card) {
    pickupCards.forEach((c) => {
      c.classList.toggle('active', c === card);
      c.setAttribute('aria-selected', String(c === card));
    });

    state.pickupName    = card.dataset.pickupName    || '';
    state.pickupAddress = card.dataset.pickupAddress || '';

    /* MAJ du récap point sélectionné */
    if ($id('pickupSummaryName'))    $id('pickupSummaryName').textContent    = state.pickupName;
    if ($id('pickupSummaryAddress')) $id('pickupSummaryAddress').textContent = state.pickupAddress;

    /* MAJ hidden field */
    if ($id('hiddenPickupPoint')) {
      $id('hiddenPickupPoint').value = `${state.pickupName} — ${state.pickupAddress}`;
    }

    track('choix_point_relais', { name: state.pickupName });
  }

  pickupCards.forEach((card) => {
    card.addEventListener('click', () => selectPickup(card));
    card.addEventListener('keydown', (e) => {
      if (e.key === 'Enter' || e.key === ' ') {
        e.preventDefault();
        selectPickup(card);
  ...
  /* Bouton "Actualiser" les points relais */
  $id('pickupRefresh')?.addEventListener('click', () => {
    /* Placeholder — à connecter à l'API Mondial Relay */
    const btn = $id('pickupRefresh');
    if (btn) {
      btn.disabled = true;
      btn.textContent = '...';
      setTimeout(() => {
        btn.disabled = false;
        btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/></svg> Actualiser`;
      }, 1200);
    }
  });
}
...
/* ─────────────────────────────────────────────────────────────
   ...
   Modifiez CONFIG.stripeLink en haut de ce fichier pour changer le lien.
───────────────────────────────────────────────────────────── */
...
function initReset() {
  $id('resetAllBtn')?.addEventListener('click', () => {
    ...
    state.pickupName    = 'Tabac Presse du Centre';
    state.pickupAddress = '12 Rue de la République, 75001 Paris';
    ...
    if ($id('pickupShell')) $id('pickupShell').hidden = false;
...
function init() {
  ...
  initAutosave();
  initDeliveryChoice();       /* ← Nouveau : choix livraison interactif */
