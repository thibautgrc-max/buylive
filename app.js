...
const DOM = {
  ...
  hiddenCarrierCode:      () => $id('hiddenCarrierCode') || $id('carrier_code'),
  hiddenCarrierName:      () => $id('hiddenCarrierName') || $id('carrier_name'),
  hiddenDeliveryType:     () => $id('hiddenDeliveryType') || $id('delivery_type'),
  hiddenPickupPointId:    () => $id('hiddenPickupPointId') || $id('pickup_point_id'),
  hiddenPickupPointName:  () => $id('hiddenPickupPointName') || $id('pickup_point_name'),
  hiddenPickupPointAddr:  () => $id('hiddenPickupPointAddress') || $id('pickup_point_address'),
  hiddenPickupPointZip:   () => $id('hiddenPickupPointZip') || $id('pickup_point_zip'),
  hiddenPickupPointCity:  () => $id('hiddenPickupPointCity') || $id('pickup_point_city'),
  hiddenPickupPointDist:  () => $id('hiddenPickupPointDistance') || $id('pickup_point_distance'),
  hiddenPickupPointLabel: () => $id('hiddenPickupPointLabel') || $id('pickup_point_label'),

  // Enhanced delivery UI (optional / future-safe)
  deliveryModeBtns:  () => $qsa('[data-delivery-mode]'),
  carrierBtns:       () => $qsa('[data-carrier], [data-carrier-code]'),
  pickupPanel:       () => $id('pickupShell') || $id('pickupPanel') || $id('pickup-panel'),
  ...
  pickupSelectedBox: () => $id('pickupSummary') || $id('pickupSelectedBox') || $id('pickup-selected-box'),
  pickupSelectedName:() => $id('pickupSummaryTitle') || $id('pickupSelectedName') || $id('pickup-selected-name'),
  pickupSelectedMeta:() => $id('pickupSummaryAddress') || $id('pickupSelectedMeta') || $id('pickup-selected-meta'),
...
function bindStepNextButtons() {
  ...
  DOM.step2Next()?.addEventListener('click', () => {
    ...
    setTimeout(() => {
      ...
      if (state.deliveryModuleEnabled && state.deliveryMode !== 'home' && canLoadPickupPoints()) {
...
function initEnhancedDeliveryModule() {
  ...
  DOM.carrierBtns().forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      state.carrierCode = btn.dataset.carrier || btn.dataset.carrierCode || CONFIG.delivery.defaultCarrier;
  ...
  document.getElementById('pickupRefreshBtn')?.addEventListener('click', () => {
    if (state.deliveryMode !== 'home') loadPickupPoints();
  });
...
function setPickupLoading(isLoading) {
  ...
  if (status) {
    status.textContent = isLoading ? 'Recherche des points proches…' : '';
    status.classList.toggle('is-loading', isLoading);
    if (isLoading) status.classList.remove('is-error');
  }
}
...
function setPickupEmpty(visible, message = '') {
  ...
  if (status) {
    if (visible && message) status.textContent = message;
    status.classList.toggle('is-error', visible && !!message);
    if (!(visible && message)) status.classList.remove('is-error');
  }
}
...
async function loadPickupPoints() {
  ...
  try {
    ...
    if (DOM.pickupStatus()) {
      ...
      DOM.pickupStatus().classList.remove('is-error', 'is-loading');
    }
  } catch (err) {
...
function renderPickupList(points) {
  ...
  points.forEach(point => {
    ...
    item.innerHTML = `
      <span class="pickup-card-head">
        <strong class="pickup-card-title">${escapeHtml(point.name)}</strong>
        <span class="pickup-card-type">${point.type === 'locker' ? 'Locker' : 'Point relais'}</span>
      </span>
      <span class="pickup-card-address">${escapeHtml(point.address)}, ${escapeHtml(point.zip)} ${escapeHtml(point.city)}</span>
      <span class="pickup-card-foot">
        <span class="pickup-card-distance">${fmt.km(point.distanceKm)}</span>
        <span class="pickup-card-hours">${point.hours ? escapeHtml(point.hours) : escapeHtml(point.carrierName)}</span>
      </span>
    `;
...
function highlightPickupListSelection() {
  ...
  list.querySelectorAll('.pickup-card').forEach(el => {
...
function renderPickupSummary() {
  ...
  if (meta) {
    meta.textContent = `${state.pickupPoint.address}, ${state.pickupPoint.zip} ${state.pickupPoint.city}`;
  }

  const distanceEl = document.getElementById('pickupSummaryDistance');
  if (distanceEl) distanceEl.textContent = fmt.km(state.pickupPoint.distanceKm);
}
...
function resetAll() {
  [
    ...
    'hiddenCarrierCode','hiddenCarrierName','hiddenDeliveryType','hiddenPickupPointId',
    'hiddenPickupPointName','hiddenPickupPointAddress','hiddenPickupPointZip',
    'hiddenPickupPointCity','hiddenPickupPointDistance','hiddenPickupPointLabel',
  ...
  DOM.carrierBtns().forEach((b, i) => {
    const isDefault = ((b.dataset.carrier || b.dataset.carrierCode || '') === CONFIG.delivery.defaultCarrier) || i === 0;
    b.classList.toggle('active', isDefault);
    b.setAttribute('aria-pressed', isDefault ? 'true' : 'false');
    b.setAttribute('aria-checked', isDefault ? 'true' : 'false');
  });
