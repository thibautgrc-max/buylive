...
function initEvents() {
  ...
  /* ── Submit : Formspree → redirection Stripe ──────────────────────────────
     FLUX :
       1. Validation complète de tous les steps
       2. Synchronisation de l'état dans le DOM (render)
       3. Construction du FormData à partir du formulaire
       4. Surcharge forcée de tous les champs critiques via data.set()
          (garantit la cohérence indépendamment des valeurs DOM)
       5. Envoi fetch vers l'endpoint Formspree lu depuis form.action
       6. Si Stripe configuré → redirection avec client_reference_id
          Si Stripe non configuré → confirmation visuelle, pas de crash
     ────────────────────────────────────────────────────────────────────── */
  $id('orderForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      /* Champs produit */
      data.set('reference',         ($id('reference')?.value || '').trim());
      data.set('prix_unitaire',     state.productPrice.toFixed(2));
      data.set('quantite',          String(state.quantity));
      data.set('montant_produit',   state.productPrice.toFixed(2));
      ...
      track('formspree_ok', {
        reference_commande: state.referenceCommande,
        total:              state.total,
      ...
      if (STRIPE_CONFIGURED) {
        ...
        /* ── Stripe non encore configuré : confirmation visuelle propre ───────
           ...
           ─────────────────────────────────────────────────────────────────── */
        track('formspree_ok_stripe_pending', { reference_commande: state.referenceCommande });

        if (txt) txt.textContent = '✓ Données reçues — Ref : ' + state.referenceCommande;
