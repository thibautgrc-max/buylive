...
const CONFIG = {
  ...
  /* Endpoints — remplacez avant mise en ligne */
...
function track(event, params = {}) {
  ...
  // GA4 — décommentez si window.gtag est branché
  // if (typeof gtag !== 'undefined') gtag('event', event, payload);

  // Meta Pixel — décommentez si window.fbq est branché
  // if (typeof fbq !== 'undefined') fbq('track', event, payload);

  // TikTok Pixel — décommentez si window.ttq est branché
  // if (typeof ttq !== 'undefined') ttq.track(event, payload);

  // Debug en dev — retirez en production
  // console.debug('[track]', event, payload);
}

/* ─────────────────────────────────────────────────────────────
   INTENT ID — Identifiant unique de commande (reconciliation)
   Format : LC-{timestamp}-{random4} ex: LC-1716900000000-A3F2
───────────────────────────────────────────────────────────── */
...
function goToStep(n) {
  ...
  for (let i = 1; i <= 4; i++) {
    ...
    // Rendre uniquement les étapes déjà validées accessibles au clavier
  ...
  // Analytics : vue d'une étape
  const stepNames = { 1: 'vue_step_produit', 2: 'vue_step_coordonnees', 3: 'vue_step_adresse', 4: 'vue_step_paiement' };
...
function initReset() {
  $id('resetAllBtn')?.addEventListener('click', () => {
    ...
    // Nouvel intent ID à chaque reset (= nouvelle tentative de commande)
...
function initEvents() {
  ...
  $id('mbbCta')?.addEventListener('click', () => {
    if (state.currentStep < 4) {
      ...
      // Step 4 : déclenche directement le submit (pas juste un scroll)
      $id('submitBtn')?.click();
    }
  });
  ...
  $id('orderForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      track('clic_paiement_stripe', { total: state.total, intent_id: state.intentId });
      ...
      error.textContent = 'Une erreur est survenue. Veuillez vérifier votre connexion et réessayer.';
  ...
  // Navigation clavier sur les étapes (retour vers une étape déjà validée)
...
function handleStripeReturn() {
  ...
  if (params.get('success') === '1') {
    ...
    banner.innerHTML = `
      ...
      <span><strong>Commande confirmée&nbsp;!</strong> Un email de confirmation vous sera envoyé sous peu.</span>
    `;
    ...
    banner.innerHTML = `
      <span class="srb-icon" aria-hidden="true">←</span>
      <span><strong>Paiement non finalisé.</strong> Votre article n'est pas réservé. Vous pouvez relancer le paiement ci-dessous.</span>
    `;
    track('retour_stripe_annulation');
    window.history.replaceState({}, '', window.location.pathname);
    // Ramener sur l'étape 4 pour faciliter la relance
    setTimeout(() => goToStep(4), 80);
  }
}

function init() {
  // ── Intent ID : 1 seul par session, stable même si rechargement
  state.intentId = sessionStorage.getItem('lo_intent') || generateIntentId();
  sessionStorage.setItem('lo_intent', state.intentId);

  // ── Injection des liens sociaux depuis CONFIG (single source of truth)
  const socialMap = [
  ...
  // handleReference() est déclenché par l'event 'input' — pas d'appel à vide au init
  // sauf si une valeur a été restaurée par autosave
