...
const state = {
  ...
  /* Référence unique commande — format VH-AAAA-MM-JJ-HHMMSS-XXXX
     Générée automatiquement dans init(), stable sur la session, réinitialisée au reset. */
  referenceCommande: '',
...
───────────────────────────────────────────────────────────── */
function generateReferenceCommande() {
  const now  = new Date();
  const pad  = (n, len = 2) => String(n).padStart(len, '0');
  const date = `${now.getFullYear()}-${pad(now.getMonth() + 1)}-${pad(now.getDate())}`;
  const time = `${pad(now.getHours())}${pad(now.getMinutes())}${pad(now.getSeconds())}`;
  const rand = Math.random().toString(36).substring(2, 6).toUpperCase();
  return `VH-${date}-${time}-${rand}`;
}
...
function render() {
  ...
  const pairs = {
    ...
    /* ── Champs de suivi commande ── */
    hiddenRefCommande:      state.referenceCommande,
...
function initReset() {
  $id('resetAllBtn')?.addEventListener('click', () => {
    ...
    state.referenceCommande = generateReferenceCommande();
    sessionStorage.setItem('lo_intent',       state.intentId);
    sessionStorage.setItem('lo_ref_commande', state.referenceCommande);
...
function initEvents() {
  ...
  $id('orderForm')?.addEventListener('submit', async (e) => {
    ...
    try {
      ...
      /* ── Statuts de commande — forcés ici, jamais en amont ──
         La règle absolue :
         - AVANT Stripe → statut_commande = "Client en attente de paiement"
         - APRÈS Stripe → statut_commande = "Client payé" (webhook uniquement)
         Ces data.set() écrasent les valeurs HTML hidden pour garantir la cohérence.
      ──────────────────────────────────────────────────────── */
      data.set('reference_commande', state.referenceCommande);
      data.set('statut_commande',    'Client en attente de paiement');
      data.set('statut_logistique',  ''); // vide — défini uniquement après confirmation Stripe

      /* ── LIAISON STRIPE (à compléter quand votre lien Stripe sera personnalisé) ──
         Pour relier cette référence au paiement Stripe :
         1. Ajoutez ?client_reference_id=${state.referenceCommande} à CONFIG.stripeLink
         2. Ou passez reference_commande dans les metadata de votre Payment Link Stripe
         Exemple : CONFIG.stripeLink + '?client_reference_id=' + state.referenceCommande
         → Ainsi, dans votre dashboard Stripe, chaque paiement affiche la référence VH-...
         → Votre webhook Stripe pourra ensuite mettre à jour Formspree :
           statut_commande   = "Client payé"
           statut_logistique = "À expédier"
      ──────────────────────────────────────────────────────── */
      const stripeUrlFinal = CONFIG.stripeLink +
        (CONFIG.stripeLink.includes('?') ? '&' : '?') +
        'client_reference_id=' + encodeURIComponent(state.referenceCommande);
      ...
      /* Redirection vers Stripe avec reference_commande en client_reference_id */
...
function handleStripeReturn() {
  ...
  if (params.get('success') === '1') {
    ...
    ──────────────────────────────────────────────────────────────── */
    const refConfirmee = sessionStorage.getItem('lo_ref_commande') || '';
    banner.hidden    = false;
    ...
    banner.innerHTML = `
      ...
      <span>
        ...
        ${refConfirmee ? `<br><small>Référence&nbsp;: <strong>${refConfirmee}</strong></small>` : ''}
        Un email de confirmation vous sera envoyé sous peu.
      </span>
    `;
    track('retour_stripe_succes', { reference_commande: refConfirmee });
...
function init() {
  ...
  /* Référence commande : générée une seule fois par session de commande
     Format VH-AAAA-MM-JJ-HHMMSS-XXXX — envoyée à Formspree + Stripe */
  state.referenceCommande = sessionStorage.getItem('lo_ref_commande') || generateReferenceCommande();
  sessionStorage.setItem('lo_ref_commande', state.referenceCommande);
